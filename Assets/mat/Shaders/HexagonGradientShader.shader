...
Shader "Custom/SpriteGradient" {
 ...
 #include "UnityCG.cginc"
     ...
     v2f vert(appdata_t IN)
     {
        ...
        ////float f =  sqrt((IN.texcoord.x * IN.texcoord.x) + (IN.texcoord.y * IN.texcoord.y));
        //float k = (_Slide - 0.5) * 2;
        OUT.color = lerp(_Color, _Color2, 0.5);
        return OUT;
     }
     ...
     sampler2D _MainTex;
         fixed4 frag(v2f i) : COLOR
         {
             ...
             float k = (_Slide - 0.5) * 2;
             float multiplier = lerp(_Color, _Color2, t + k);
             fixed4 c = tex2D(_MainTex, i.texcoord) * multiplier * i.color;
             return c;
         }
         ENDCG
     }
