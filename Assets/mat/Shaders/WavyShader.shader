...
Properties
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				float moddedT = _Time[1] % _TimeDamper;
				float2 offset = float2(
						tex2D(_NoiseTex, float2(abs(IN.worldPosition.y) / _DistortionSpreader, moddedT/_TimeDamper)).r,
						tex2D(_NoiseTex, float2(moddedT/_TimeDamper, abs(IN.worldPosition.x) / _DistortionSpreader)).r );
